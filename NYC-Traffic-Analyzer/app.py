...
if map_data["last_clicked"] is not None:
    ...
    # Get the road name and city.
    place = get_place_info(latitude, longitude)

    # Display the road name and city.
    ...
    if traffic is None:
        ...
        st.metric(
            "Current Speed",
            f"{current_speed_mph:.1f} mph"
        )

        st.metric(
            "Free Flow Speed",
            f"{free_flow_speed_mph:.1f} mph"
        )
        ...
        # Explain the traffic information.
...
**Confidence**  
A number between 0 and 1 showing how confident TomTom is in the data.
